    ...
    <div className="flex justify-between gap-1.5">
      {week.map((day) => (
        <div key={day.dateKey} className="flex flex-col items-center gap-1 flex-1">
          <span className="text-[11px] font-semibold text-ink/40 lang-en">{day.label_en}</span>
          <span className="text-[11px] font-semibold text-ink/40 lang-hi">{day.label_hi}</span>
          <div
            ...
            } ${day.isToday ? "ring-2 ring-clay ring-offset-2 ring-offset-white" : ""}`}
            title={day.dateKey}
          >
            {day.active ? "✓" : ""}
          </div>

    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-4 shadow-sm">
        <label className="block mb-3">
          ...
          <p className="text-sm text-slate-500 mt-1 lang-en">{painLabels_en[pain]}</p>
          <p className="text-sm text-slate-500 mt-1 lang-hi">{painLabels_hi[pain]}</p>
        </label>
        ...
        <label className="block">
          ...
          <textarea
            ...
            rows={3}
            className="w-full mt-1 border border-slate-300 rounded-lg p-2 text-base"
      ...
      <div className="mb-4">
        <Collapsible icon="🩹" title_en="Comfort Tips" title_hi="आराम के उपाय">
          <p className="text-sm text-slate-500 mt-2 mb-3 lang-en">
            Non-medicine ways to ease pain. Follow your doctor's instructions
            for medicines.
          </p>
          <p className="text-sm text-slate-500 mt-2 mb-3 lang-hi">
            दर्द कम करने के गैर-दवा उपाय। दवाओं के लिए डॉक्टर की सलाह मानें।
          </p>
          <ul className="space-y-3">
            {painTips.map((tip) => (
              <li key={tip.title_en} className="border-l-4 border-amber-300 pl-3">
                ...
                <p className="text-sm text-slate-400 lang-hi">{tip.text_hi}</p>
              </li>
            ))}
          </ul>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
        ...
        {history.length === 0 ? (
          <>
            <p className="text-sm text-slate-400 lang-en">
              No entries yet. Save today's check-in to start.
            </p>
            <p className="text-sm text-slate-400 lang-hi">
          ...
          <ul className="space-y-3">
            {history.map((h) => (
              ...
              >
                ...
                {h.notes && <p className="mt-1 italic text-slate-500">"{h.notes}"</p>}
              </li>
            ))}
          </ul>
        )}
